    ...
    if (is_array($box_array) || is_object($box_array))  {
      foreach ($box_array as $this_date) {
       ...
       if($date_fulldate != '01.01.1970') { /* jos pvm on väärä niin ei tulosteta */
        if(!isset($_GET["air_pressure"])) {
         echo "['".$date_day." (".$date_date.")', ".$get_rows.", ".$temperature.", ".$average_air.", ".$average_humidity."],";
        } else {
         echo "['".$date_day." (".$date_date.")', ".$get_rows.", ".$temperature.", ".$average_humidity."],";
        }
       }
...
<html>
  <head>
    <title>Daily Charts - Organized Chaos</title>
    <meta charset="utf-8">
...
<div class="container body-content">
    <h3>Statistics</h3>
    <p>Here you can see the average for every single day that is selected or for a specific device.</p>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          ['Day', 'Total Entries', 'Average Temperature (C)', 'Average Air Pressure (kPa)', 'Average Humidity (%)'],
          <?php } else { ?>
          ['Day', 'Total Entries', 'Average Temperature (C)', 'Average Humidity (%)'],
          <?php
          ...
          ?>
          <?php if(!isset($_GET["air_pressure"])) { ?>
          ['<?php echo 'Today ('.$current_date.')'; ?>',  <?php echo $get_rows; ?>, <?php echo $average_temp; ?>, <?php echo $average_air; ?>, <?php echo $average_humidity; ?>],
          <?php } else { ?>
          ['<?php echo 'Today ('.$current_date.')'; ?>',  <?php echo $get_rows; ?>, <?php echo $average_temp; ?>, <?php echo $average_humidity; ?>],
          <?php } ?>
        ]);
